...
abstract contract ExhibitionConfig is ExhibitionBase {
    ...
    function setUSDXTokenAddress(address _USDXTokenAddress) external onlyOwner {
        if (_USDXTokenAddress == address(0)) revert ZeroAddress();
        USDXTokenAddress = _USDXTokenAddress;
        emit NexusUSDAddressSet(_USDXTokenAddress);
    }
    ...
    function setFaucetAmountUSDX(uint256 _amount) public onlyOwner {
        faucetAmountUSDX = _amount;
    }

    function setFaucetCooldown(uint256 _blocks) public onlyOwner {
        faucetCooldownBlocks = _blocks;
    }
}