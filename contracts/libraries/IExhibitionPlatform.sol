...
error InvalidStartBlocks();
error InvalidDurationBlocks();
error InvalidEndBlocks();
...
enum ProjectStatus {
    ...
    Active,          // 1: Tokens deposited; accepts contributions once start block is reached
    Successful,      // 2: Funding goal met; tokens claimable, owner has 7 days (in blocks) to add liquidity
    Failed,          // 3: Soft cap not reached by end block; eligible for refunds
...
struct Project {
    ...
    address contributionTokenAddress; // The ERC20 token used for contributions (e.g., USDX, exNEX)
    ...
    uint256 tokenPrice;               // The price of 1 project token in contribution tokens (ALWAYS in 18-decimal format, e.g., 0.001 = 1000000000000000).
    uint256 startBlock;               // Block number when contributions open
    uint256 endBlock;                 // Block number when contributions close
    ...
    uint256 lockDurationBlocks;       // Duration in blocks for which the initial liquidity will be locked in the AMM
    ...
    bool vestingEnabled;
    uint256 vestingCliffBlocks;       // In blocks, relative to project startBlock
    uint256 vestingDurationBlocks;    // In blocks, relative to project startBlock
    uint256 vestingIntervalBlocks;    // In blocks
    uint256 vestingInitialRelease;    // Percentage (e.g., 2000 for 20.00%)
    uint256 tokensSold;               // snapshotted at finalization
    uint256 unsoldTokensWithdrawn;    // snapshotted at withdrawal
}
...
struct VestingInfo {
    ...
    uint256 startBlock;       // Block number when vesting starts for this user (after cliff)
    uint256 lastClaimBlock;   // Last block at which the user claimed vested tokens
    uint256 nextClaimBlock;   // Block number when the next portion of tokens becomes claimable
}
...
interface IExhibition {
    ...
    function setUSDXTokenAddress(address _USDXTokenAddress) external; 
    function setFaucetAmounts(uint256 _exhAmount, uint256 _usdtAmount) external;
    function setFaucetCooldown(uint256 _cooldownBlocks) external;
    ...
    function faucetCooldownBlocks() external view returns (uint256);
    function lastFaucetRequestBlock(address user) external view returns (uint256);
    ...
    function isEmergencyRefundAvailable(uint256 projectId) external view returns (bool available, uint256 deadlineBlock, uint256 blocksRemaining);
    ...
    function projects(uint256 projectId) external view returns (
        ...
        bool liquidityAdded,
        bool vestingEnabled,
        uint256 vestingCliffBlocks,
        uint256 vestingDurationBlocks,
        uint256 vestingIntervalBlocks,
        uint256 vestingInitialRelease,
        uint256 tokensSold,
        uint256 unsoldTokensWithdrawn
    );
    ...
    event NexusUSDAddressSet(address indexed tokenAddress); 
...
/**
 * @title INexusUSD
 * @dev Interface for the NexusUSD token
 */
interface INexusUSD {
    function mint(address to, uint256 amount) external;
    function transferOwnership(address newOwner) external;
