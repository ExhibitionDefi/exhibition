...
abstract contract ExhibitionAMMFees is ExhibitionAMMStorage, ExhibitionAMMErrors {
     ...
     * @param _tradingFeeBps Trading fee in basis points (e.g., 25 = 0.25%)
    ...
    function _processSwapFees(
        ...
        if (tokenIn == token0) {
            totalFeesCollected[token0][token1] += tradingFeeAmount;   // token0 fees
        } else {
            totalFeesCollectedToken1[token0][token1] += tradingFeeAmount; // token1 fees
        }
        lastFeeUpdateBlock[token0][token1] = block.number;
    ...
    {
        (address token0, address token1) = ExhibitionAMMLibrary.sortTokens(tokenA, tokenB);
        totalFees0 = totalFeesCollected[token0][token1];
        totalFees1 = totalFeesCollectedToken1[token0][token1];
    }
}