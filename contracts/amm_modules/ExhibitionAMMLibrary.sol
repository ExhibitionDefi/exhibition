...
library ExhibitionAMMLibrary {
    ...
    function calculatePriceImpact(
        ...
        // Spot price: reserveOut per reserveIn (output per unit input)
        uint256 spotPrice = (reserveOut * 1e18) / reserveIn;

        // Get amount out after fees
        (uint256 amountOut,) = getAmountOut(amountIn, reserveIn, reserveOut, tradingFeeBps);
        if (amountOut == 0) return 0;

        // Effective price: amountOut per amountIn (output per unit input) — same direction as spotPrice
        uint256 effectivePrice = (amountOut * 1e18) / amountIn;

        // Impact: how much worse effectivePrice is vs spotPrice
        if (effectivePrice < spotPrice) {
            impactPercentage = ((spotPrice - effectivePrice) * 10000) / spotPrice;
        }
    }
