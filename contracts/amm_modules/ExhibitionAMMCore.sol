// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/utils/ReentrancyGuard.sol";
import "./ExhibitionAMMStorage.sol";
import "./ExhibitionAMMLibrary.sol";
import "./ExhibitionAMMFees.sol";
import "./ExhibitionAMMLocks.sol";
import "./ExhibitionAMMEarnings.sol";
import "../libraries/IExhibitionMinimal.sol";

/**
 * @title ExhibitionAMMCore
 * @dev Core AMM logic - liquidity and swap functions.
 *
 *  Earnings model (current architecture):
 *
 *  • Global accounting:
 *      - Each pool tracks `accFeePerLP0` and `accFeePerLP1`
 *        (cumulative fees earned per LP token, scaled to 1e18).
 *
 *  • User accounting:
 *      - Each user stores `feeDebt0` and `feeDebt1`, representing
 *        their last checkpoint of accumulated fees.
 *      - Pending rewards are computed as:
 *
 *          pending = (lpBalance * accFeePerLP) - feeDebt
 *
 *  • Lifecycle hooks:
 *
 *      - _addLiquidity:
 *          1. `_updateRewards` is called to settle pending earnings
 *          2. LP balance increases
 *          3. `feeDebt` is reset to the new baseline
 *
 *      - _removeLiquidity:
 *          1. `_updateRewards` is called before LP burn
 *          2. LP balance decreases
 *          3. `feeDebt` is reset to the new baseline
 *
 *      - _swapTokenForToken:
 *          1. Fees are split into:
 *              • protocolFee → tracked separately
 *              • lpFee       → distributed via `accFeePerLP`
 *          2. LP fees are added using `_updatePoolFees`
 *
 *  • Design properties:
 *      - O(1) per operation (no loops, no per-user iteration)
 *      - No snapshots or historical arrays
 *      - Gas-efficient and scalable for large LP sets
 *      - Deterministic reward calculation (no timing exploits)
 */
abstract contract ExhibitionAMMCore is
    ExhibitionAMMStorage,
    ExhibitionAMMFees,
    ExhibitionAMMLocks,
    ExhibitionAMMEarnings,
    ReentrancyGuard
{

    // ================================
    //       Error Definitions
    // ================================

    error InvalidAmount();
    error InsufficientLiquidity();
    error SlippageTooHigh();
    error DeadlineExpired();
    error PoolDoesNotExist();
    error PoolAlreadyExists();
    error ZeroLiquidity();
    error InvalidTokenAddress();
    error InvalidRecipient();
    error UnauthorizedPoolCreation();

    // ================================
    //       Internal Helpers
    // ================================

    function _transferTokens(
        address _token,
        address _from,
        address _to,
        uint256 _amount
    ) internal {
        if (_amount == 0) revert ExhibitionAMMLibrary.ZeroAmount();
        if (_to == address(0)) revert InvalidRecipient();

        if (_from == address(this)) {
            bool success = IERC20(_token).transfer(_to, _amount);
            if (!success) revert TokenTransferFailed();
        } else {
            bool success = IERC20(_token).transferFrom(_from, _to, _amount);
            if (!success) revert TokenTransferFailed();
        }
    }

    function _mintLiquidity(
        address _tokenA,
        address _tokenB,
        address _to,
        uint256 _amount
    ) internal {
        if (address(exhibitionLPTokens) == address(0)) revert Unauthorized();
        if (_amount == 0) revert ExhibitionAMMLibrary.ZeroAmount();

        (address token0, address token1) = ExhibitionAMMLibrary.sortTokens(_tokenA, _tokenB);

        if (!userHasPosition[token0][token1][_to]) {
            userPoolTokensA[_to].push(token0);
            userPoolTokensB[_to].push(token1);
            userHasPosition[token0][token1][_to] = true;
        }

        exhibitionLPTokens.mint(token0, token1, _to, _amount);
        // Single source of truth — read back from LP contract
        liquidityPools[token0][token1].totalLPSupply = exhibitionLPTokens.totalSupply(token0, token1);

       // ✅ Sync lpBalance
       lpPositions[_to][token0][token1].lpBalance = exhibitionLPTokens.balanceOf(token0, token1, _to);
    }

   function _burnLiquidity(
        address _tokenA,
        address _tokenB,
        address _from,
        uint256 _amount
    ) internal {
        if (address(exhibitionLPTokens) == address(0)) revert Unauthorized();
        if (_amount == 0) revert ExhibitionAMMLibrary.ZeroAmount();

        (address token0, address token1) = ExhibitionAMMLibrary.sortTokens(_tokenA, _tokenB);

        exhibitionLPTokens.burn(token0, token1, _from, _amount);
        // Single source of truth — read back from LP contract
        liquidityPools[token0][token1].totalLPSupply = exhibitionLPTokens.totalSupply(token0, token1);

        // ✅ Sync lpBalance
        lpPositions[_from][token0][token1].lpBalance = exhibitionLPTokens.balanceOf(token0, token1, _from);
    }

    function _updateReserves(
        address _token0,
        address _token1,
        uint256 _newReserve0,
        uint256 _newReserve1
    ) internal {
        LiquidityPool storage pool = liquidityPools[_token0][_token1];
        TWAPData storage twap      = twapData[_token0][_token1];

        uint32 blockNumber   = uint32(block.number);
        uint32 blocksElapsed = blockNumber - twap.blockNumberLast;

        // Accumulate price if at least one block has passed and reserves are valid
        if (blocksElapsed > 0 && pool.reserveA != 0 && pool.reserveB != 0) {
            uint256 price0 = ExhibitionAMMLibrary.mulDiv(pool.reserveB, 1e18, pool.reserveA);
            uint256 price1 = ExhibitionAMMLibrary.mulDiv(pool.reserveA, 1e18, pool.reserveB);

            twap.price0CumulativeLast += price0 * blocksElapsed;
            twap.price1CumulativeLast += price1 * blocksElapsed;
            twap.blocksAccumulated    += blocksElapsed;
        }

        twap.blockNumberLast = blockNumber;

        // Update reserves
        pool.reserveA = _newReserve0;
        pool.reserveB = _newReserve1;
        pool.kLast    = _newReserve0 * _newReserve1;

        emit ReservesUpdated(_token0, _token1, _newReserve0, _newReserve1);
    }

    function _getReserves(address _tokenA, address _tokenB)
        internal
        view
        returns (uint256 reserveA, uint256 reserveB)
    {
        (address token0, address token1) = ExhibitionAMMLibrary.sortTokens(_tokenA, _tokenB);

        if (!poolExists[token0][token1]) revert PoolDoesNotExist();

        LiquidityPool storage pool = liquidityPools[token0][token1];

        if (_tokenA == token0) {
            reserveA = pool.reserveA;
            reserveB = pool.reserveB;
        } else {
            reserveA = pool.reserveB;
            reserveB = pool.reserveA;
        }
    }

    // ================================
    //       Add Liquidity
    // ================================

    /**
     * @dev Add liquidity to a pool, creating it if it does not yet exist.
     *
     *  Earnings integration:
     *
     *  • `_updateRewards` is called BEFORE modifying the user's LP balance.
     *    This settles any pending fee earnings based on the previous balance.
     *
     *  • After liquidity is added:
     *      - User LP balance increases
     *      - `feeDebt` is reset to match the new balance and current
     *        `accFeePerLP` values
     *
     *  This ensures the user only earns fees generated AFTER this deposit.
     */
    function _addLiquidity(
        address _tokenA,
        address _tokenB,
        uint256 _amountADesired,
        uint256 _amountBDesired,
        uint256 _amountAMin,
        uint256 _amountBMin,
        address _to,
        uint256 _deadline
    ) internal returns (uint256 amountA, uint256 amountB, uint256 liquidity) {
        if (block.number >= _deadline) revert DeadlineExpired();
        if (_to == address(0)) revert InvalidRecipient();

        (address token0, address token1) = ExhibitionAMMLibrary.sortTokens(_tokenA, _tokenB);

        uint256 newReserve0;
        uint256 newReserve1;

        // Create pool if it doesn't exist
        if (!poolExists[token0][token1]) {
            _createPool(token0, token1);

            if (_amountADesired == 0 || _amountBDesired == 0) revert InvalidAmount();

            amountA = _amountADesired;
            amountB = _amountBDesired;
            liquidity = ExhibitionAMMLibrary.sqrt(amountA * amountB);
            if (liquidity == 0) revert ZeroLiquidity();

            newReserve0 = (_tokenA == token0) ? amountA : amountB;
            newReserve1 = (_tokenA == token0) ? amountB : amountA;

        } else {
            // Existing pool
            (uint256 reserveA, uint256 reserveB) = _getReserves(_tokenA, _tokenB);

            (amountA, amountB) = ExhibitionAMMLibrary.calculateOptimalAmounts(
                _amountADesired,
                _amountBDesired,
                reserveA,
                reserveB
            );

            if (amountA < _amountAMin || amountB < _amountBMin) revert SlippageTooHigh();

            uint256 totalSupply = liquidityPools[token0][token1].totalLPSupply;
            liquidity = ExhibitionAMMLibrary.calculateLiquidity(
                amountA,
                amountB,
                reserveA,
                reserveB,
                totalSupply
            );
            if (liquidity == 0) revert ZeroLiquidity();

            newReserve0 = (_tokenA == token0) ? reserveA + amountA : reserveB + amountB;
            newReserve1 = (_tokenA == token0) ? reserveB + amountB : reserveA + amountA;
        }

        // Collect past fees for _to before changing LP balance
        _updateRewards(_to, token0, token1);

        // Transfer tokens into the pool
        _transferTokens(_tokenA, msg.sender, address(this), amountA);
        _transferTokens(_tokenB, msg.sender, address(this), amountB);

        // Mint LP tokens to user
        _mintLiquidity(token0, token1, _to, liquidity);

        // Reset user's feeDebt to the new LP balance
        LPPosition storage pos = lpPositions[_to][token0][token1];
        pos.feeDebt0 = (pos.lpBalance * liquidityPools[token0][token1].accFeePerLP0) / 1e18;
        pos.feeDebt1 = (pos.lpBalance * liquidityPools[token0][token1].accFeePerLP1) / 1e18;

        // Update reserves using precomputed values
        _updateReserves(token0, token1, newReserve0, newReserve1);

        emit LiquidityAdded(msg.sender, _tokenA, _tokenB, amountA, amountB, liquidity);
    }

    function _createPool(address token0, address token1) internal {
        if (exhibitionContract != address(0)) {
            bool token0IsProject = IExhibitionMinimal(exhibitionContract).isProjectToken(token0);
            bool token1IsProject = IExhibitionMinimal(exhibitionContract).isProjectToken(token1);

            if (token0IsProject || token1IsProject) {
                if (msg.sender != exhibitionContract) {
                    revert UnauthorizedPoolCreation();
                }
            }
        }

        poolExists[token0][token1]                    = true;
        liquidityPools[token0][token1].tokenA         = token0;
        liquidityPools[token0][token1].tokenB         = token1;
        liquidityPools[token0][token1].inceptionBlock = block.number;

        allPoolPairs.push([token0, token1]);

        emit PoolCreated(token0, token1, block.number);
    }

    // ================================
    //       Remove Liquidity
    // ================================

    /**
     * @dev Remove liquidity from a pool.
     *
     *  Earnings integration:
     *
     *  • `_updateRewards` is called BEFORE LP tokens are burned.
     *    This ensures all pending earnings are settled using the user's
     *    full LP balance prior to withdrawal.
     *
     *  • After liquidity is removed:
     *      - User LP balance decreases
     *      - `feeDebt` is reset to match the reduced balance and current
     *        `accFeePerLP` values
     *
     *  This prevents the user from earning fees on withdrawn liquidity.
     */
    function _removeLiquidity(
        address _tokenA,
        address _tokenB,
        uint256 _lpAmount,
        uint256 _amountAMin,
        uint256 _amountBMin,
        address _to,
        uint256 _deadline
    ) internal returns (uint256 amountA, uint256 amountB) {
        if (block.number >= _deadline) revert DeadlineExpired();
        if (_to == address(0)) revert InvalidRecipient();
        if (_lpAmount == 0) revert ZeroLiquidity();

        _checkLiquidityLock(_tokenA, _tokenB, msg.sender, _lpAmount);

        (address token0, address token1) = ExhibitionAMMLibrary.sortTokens(_tokenA, _tokenB);
        if (!poolExists[token0][token1]) revert PoolDoesNotExist();

        LiquidityPool storage pool = liquidityPools[token0][token1];
        uint256 totalSupply = pool.totalLPSupply;

        if (totalSupply == 0 || pool.reserveA == 0 || pool.reserveB == 0) {
            revert InsufficientLiquidity();
        }

        (uint256 amount0, uint256 amount1) = ExhibitionAMMLibrary.calculateRemoveAmounts(
            _lpAmount,
            pool.reserveA,
            pool.reserveB,
            totalSupply
        );

        amountA = (_tokenA == token0) ? amount0 : amount1;
        amountB = (_tokenA == token0) ? amount1 : amount0;

        if (amountA < _amountAMin || amountB < _amountBMin) revert SlippageTooHigh();

        // ── Earnings hook ──────────────────────────────────────────────────
        // Collect all pending fees for user before LP balance decreases
        _updateRewards(msg.sender, token0, token1);
        // ──────────────────────────────────────────────────────────────────

        // Burn LP tokens (decrements totalLPSupply)
        _burnLiquidity(token0, token1, msg.sender, _lpAmount);

        // Reset user's feeDebt to the new LP balance after burn
        LPPosition storage pos = lpPositions[msg.sender][token0][token1];
        pos.feeDebt0 = (pos.lpBalance * pool.accFeePerLP0) / 1e18;
        pos.feeDebt1 = (pos.lpBalance * pool.accFeePerLP1) / 1e18;

        // Transfer tokens out
        _transferTokens(_tokenA, address(this), _to, amountA);
        _transferTokens(_tokenB, address(this), _to, amountB);

        // Update reserves after transfers
        uint256 newReserve0 = pool.reserveA - amount0;
        uint256 newReserve1 = pool.reserveB - amount1;
        _updateReserves(token0, token1, newReserve0, newReserve1);

        emit LiquidityRemoved(msg.sender, _tokenA, _tokenB, amountA, amountB);
    }

    // ================================
    //       Swap
    // ================================

    /**
     * @dev Swap one token for another.
     *
     *  Fee & earnings integration:
     *
     *  • Swap fees are split into:
     *      - `protocolFee` → accumulated separately for the protocol
     *      - `lpFee`       → distributed to LPs via `accFeePerLP`
     *
     *  • LP fee distribution:
     *      - `_updatePoolFees` is called to increase
     *        `accFeePerLP0` or `accFeePerLP1`
     *      - This updates the global earnings per LP token
     *
     *  • LP earnings model:
     *      - LPs do NOT rely solely on reserve growth
     *      - Earnings are tracked explicitly via:
     *
     *          pending = (lpBalance * accFeePerLP) - feeDebt
     *
     *  • Reserve behavior:
     *      - LP fees remain in the pool (increase reserves)
     *      - Protocol fees are excluded from reserves
     *
     *  This ensures accurate, gas-efficient, and deterministic
     *  fee distribution to all LPs.
     */
    function _swapTokenForToken(
        address _tokenIn,
        address _tokenOut,
        uint256 _amountIn,
        uint256 _minAmountOut,
        address _to,
        uint256 _deadline
    ) internal returns (uint256 amountOut) {
        if (block.number >= _deadline) revert DeadlineExpired();
        if (_to == address(0)) revert InvalidRecipient();
        if (_tokenIn == address(0) || _tokenOut == address(0) || _tokenIn == _tokenOut) {
            revert InvalidTokenAddress();
        }
        if (_amountIn == 0) revert ExhibitionAMMLibrary.ZeroAmount();

        (address token0, address token1) = ExhibitionAMMLibrary.sortTokens(_tokenIn, _tokenOut);
        if (!poolExists[token0][token1]) revert PoolDoesNotExist();

        _transferTokens(_tokenIn, msg.sender, address(this), _amountIn);

        (uint256 tradingFeeAmount, uint256 protocolFeeAmount,) = _calculateSwapFees(_amountIn);

        (uint256 reserveIn, uint256 reserveOut) = _getReserves(_tokenIn, _tokenOut);

        uint256 amountInAfterFees = _amountIn - tradingFeeAmount;
        uint256 numerator         = amountInAfterFees * reserveOut;
        uint256 denominator       = reserveIn + amountInAfterFees;
        amountOut                 = numerator / denominator;

        if (amountOut < _minAmountOut) revert SlippageTooHigh();

        _processSwapFees(token0, token1, _tokenIn, tradingFeeAmount, protocolFeeAmount);

        // Distribute LP fee portion to accFeePerLP
        uint256 lpFee = tradingFeeAmount - protocolFeeAmount;
        if (lpFee > 0) {
            _accrueFees(token0, token1, _tokenIn, lpFee);
        }

        _transferTokens(_tokenOut, address(this), _to, amountOut);

        uint256 newReserveIn  = reserveIn + _amountIn - protocolFeeAmount;
        uint256 newReserveOut = reserveOut - amountOut;

        uint256 newReserve0 = (_tokenIn == token0) ? newReserveIn  : newReserveOut;
        uint256 newReserve1 = (_tokenIn == token0) ? newReserveOut : newReserveIn;

        _updateReserves(token0, token1, newReserve0, newReserve1);

        emit Swap(msg.sender, _tokenIn, _tokenOut, _amountIn, amountOut, tradingFeeAmount, protocolFeeAmount);
    }
}